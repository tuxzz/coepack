...
n_blue = 16
n_alpha = 2
n_color = n_red * n_green * n_blue * n_alpha
palette = np.zeros((n_color, 4), dtype=np.float32)
i_color = 0
for i_red in range(n_red):
  for i_green in range(n_green):
    for i_blue in range(n_blue):
      for i_alpha in range(n_alpha):
        palette[i_color, :] = (i_red / (n_red - 1), i_green / (n_green - 1), i_blue / (n_blue - 1), i_alpha / (n_alpha - 1))
        i_color += 1
...
def pal_euclidean(v):
  d = np.abs(v.reshape(1, 4) - palette)
...
def _apply_palette_ed(img, krnl):
  h, w, c = img.shape
  assert c == 4
  if krnl is not None:
    kh, kw = krnl.shape
...
def apply_palette_ed(img, krnl):
  return _apply_palette_ed(img_util.ensure_rgba(img), krnl)

def to_coe(img):
  n, c = img.shape
  assert c == 4
  img = np.round(img * (n_red - 1, n_green - 1, n_blue - 1, n_alpha - 1)).astype(np.uint16)
  img[:, 1] <<= int(np.ceil(np.log2(n_red)))
  img[:, 2] <<= int(np.ceil(np.log2(n_red))) + int(np.ceil(np.log2(n_green)))
  img[:, 3] <<= int(np.ceil(np.log2(n_red))) + int(np.ceil(np.log2(n_green))) + int(np.ceil(np.log2(n_blue)))
